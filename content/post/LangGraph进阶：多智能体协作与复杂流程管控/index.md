---
title: LangGraph进阶：多智能体协作与复杂流程管控
description: 本文介绍 LangGraph 中的多智能体协作与复杂流程管控。
date: 2026-04-25T16:34:25-08:00
draft: false
categories:
- AI
- DataWhale
...
## 1 多智能体系统（Multi-Agent Systems）核心设计
...
llm = ChatOpenAI(
    ...
    base_url=BASE_URL,
    model=MODEL,  # 注意：根据你使用的模型修改名称！！！！ 后面章节不再继续说明
    temperature=0.7,  # 随机性：0-1，越小越严谨，越大越有创造力
)
...
#### 1.2.1 中心化协作（Supervisor）：基于路由的“主管-员工”模式
...
#### 1.2.3 去中心化协作（Peer-to-peer）：基于状态触发的自主协同
...
### 2.1 子图（Subgraphs）机制：复杂系统的模块化拆解
...
#### 2.1.1 实操案例：在主流程中嵌入一个独立的子图

案例演示：主图是模拟老师收卷子，子图用来批改和统计

```python

# 【教案实操案例】2.1.2 在主流程中嵌入独立可复用的“作业批改”子图
# 核心需求（学生易理解）：
...
class HomeworkMainState(TypedDict):
    ...
    correction_result: Optional[CorrectionSubgraphState] # 子图批改结果（初始None）
    feedback: Optional[str]            # 最终批改反馈（初始None）
...
### 2.2 并行任务处理（Parallelization）：提升效率的关键
...
# =====================================================
# 第一阶段：执行到 plot 已经存盘后中断（在 review 前中断）
# =====================================================
print("\n=== 第一次运行（执行到 plot 存盘后中断）===")
...
for step in stream:
    print("当前 step：", step.keys())

    # ====================== 修复在这里 ======================
    # 等到 plot 执行完 → 交给 review 前 → 状态已存盘
    if "review" in step:
        # 获取已经持久化的最新状态
        current_state = app.get_state({"configurable": {"thread_id": thread_id}})
        plot_content = current_state.values["plot"]
        retry_count = current_state.values["retry_count"]

        print("\n🛑 程序中断（已存盘）")
        print(f"中断时版本：第 {retry_count} 版")
        print(f"中断时情节内容：\n{plot_content}")
        break  # 安全中断

# =====================================================
# 第二阶段：从 checkpoint 恢复（会复用之前的 plot）
# =====================================================
print("\n=== 第二次运行（从存档恢复）===")
...
=== 第一次运行（执行到 plot 存盘后中断）===
当前 step： dict_keys(['plot'])
当前 step： dict_keys(['review'])
...
在遥远的未来，地球早已被遗忘，星际间的文明如繁星般闪烁。主角林宇，一个年轻的宇航员，正驾驶着他的飞船“流浪者号”穿越无尽的宇宙。他的父母在一次星际探险中失踪，留下他孤身一人，心中怀揣着寻找真相的执念。

就在林宇准备降落在一个名为“阿尔法-7”的荒芜星球时，飞船的警报骤然响起。屏幕上闪烁着红色警告，显示出一艘敌对舰队正朝他逼近。原来，阿尔法-7是一个被黑暗势力控制的星球，他们正在寻找一件传说中的神器，传说中能改变整个宇宙的力量。

林宇面临着艰难的选择：是继续追寻父母的下落，还是为了拯救即将被黑暗势力吞噬的星球而战。他知道，自己的一举一动，将决定无数生命的命运。
...
#### 3.2.1 在执行前中断：关键操作（如转账、发邮件）的人工授权
...
在无尽的宇宙深处，林启坐在破旧的飞船驾驶舱内，透过窗户凝视着闪烁的星辰。自从那场突如其来的宇宙风暴摧毁了他的家园，林启便成为了流浪者，孤独地漂浮在这片浩瀚的星海中。

飞船的仪器发出微弱的警报声，显示着燃料即将耗尽的红色警告。林启叹了口气，心中暗自焦虑。他的目标是寻找一个可以栖息的新星球，但在这片荒凉的宇宙中，机会渺茫。记忆中的地球，如今只是一片模糊的幻影。

就在这时，飞船的探测器突然响起，显示前方有一个未知的星系。林启的心跳加速，或许这就是他命运的转折点。他调整航向，飞船缓缓驶向那片神秘的星域，心中燃起了一丝希望。无论前方等待他的是什么，他都将勇敢面对，继续他的星际流浪之旅。
...
在无尽的宇宙深处，包子坐在破旧的飞船驾驶舱内，透过窗户凝视着闪烁的星辰。自从那场突如其来的宇宙风暴摧毁了他的家园，包子便成为了流浪者，孤独地漂浮在这片浩瀚的星海中。

飞船的仪器发出微弱的警报声，显示着燃料即将耗尽的红色警告。包子叹了口气，心中暗自焦虑。他的目标是寻找一个可以栖息的新星球，但在这片荒凉的宇宙中，机会渺茫。记忆中的地球，如今只是一片模糊的幻影。

就在这时，飞船的探测器突然响起，显示前方有一个未知的星系。包子的心跳加速，或许这就是他命运的转折点。他调整航向，飞船缓缓驶向那片神秘的星域，心中燃起了一丝希望。无论前方等待他的是什么，他都将勇敢面对，继续他的星际流浪之旅。
...
在无垠的宇宙中，林启的飞船“星辰号”孤独地穿梭于星际之间。自从地球被毁灭后，他便成为了流浪者，寻找着失落的家园和希望。飞船的控制台上，闪烁着各个星系的坐标，然而林启的心中却充满了迷茫。

“再一次，尝试连接信号。”他自言自语，手指在屏幕上飞快滑动。过去的回忆如潮水般涌来，父母的笑容、朋友的欢声笑语，仿佛就在眼前。然而，现实却是无情的，只有冷冰冰的宇宙和无尽的孤独。

突然，屏幕上出现了一道微弱的信号，林启的心跳加速。他调整频率，试图捕捉到更多的信息。就在这时，飞船的警报骤然响起，前方出现了一道巨大的光芒，仿佛是宇宙的裂缝。林启握紧操纵杆，心中涌起一丝不安与期待。他知道，这或许是他命运的转折点。
...
### 4.4 核心步骤3：构建langgraph图（StateGraph）
